...
class SlackToBookmark:
    ...
    def __init__(self):
        ...
        logger.info(f"設定値: WORKSPACE_NAME={self.workspace_name}, WORKSPACE_ID={self.workspace_id}")
        ...
        try:
            ...
            if result.get("ok"):
                logger.info(f"API認証テスト成功: ユーザー={result.get('user')}, チーム={result.get('team')}")
                logger.info(f"APIレスポンスのワークスペース情報: team_id={result.get('team_id')}")
                
                # .envのワークスペースIDと異なる場合は警告
                if result.get('team_id') != self.workspace_id:
                    logger.warning(f"注意: .envのワークスペースID({self.workspace_id})とAPI認証のチームID({result.get('team_id')})が一致しません")
                    logger.warning("修正: .envのWORKSPACE_IDを更新することをお勧めします")
                    # APIレスポンスからの値を使用
                    self.workspace_id = result.get('team_id')
                    logger.info(f"ワークスペースIDを{self.workspace_id}に自動更新しました")
            else:
        ...
        # 設定チェック
        if not self.token:
            logger.error("SLACK_TOKEN環境変数が設定されていません。"
                        "\n解決策: "
        ...
        # 各クラスの初期化
        self.slack_client = SlackClient(self.token, self.workspace_name, self.workspace_id)
        self.bookmark_generator = BookmarkGenerator(self.workspace_name, self.workspace_id)
        self.guide_generator = GuideGenerator()
        
        logger.info("SlackToBookmark initialized")
    
    def run(self, channel_filter: Optional[List[str]] = None, public_only: bool = False, 
            include_dm: bool = True, anonymize: bool = False) -> bool:
        """
        メイン処理を実行
        ...
        generated_files = []
        try:
            logger.info(f"Processing with options: channel_filter={channel_filter}, public_only={public_only}, include_dm={include_dm}")
            ...
            if channel_filter:
                ...
                if not channels:
                    logger.warning("フィルターに一致するチャンネルが見つかりませんでした")
            
            # チャンネルブックマークの生成
            if channels:
                html_file_path = self.bookmark_generator.generate_channel_bookmarks(channels, output_file)
                if html_file_path:
                    ...
                    if guide_path:
                        ...
                        try:
                            ...
                            logger.info(f"チャンネルガイドページを開きました: {guide_path}")
                        except Exception as e:
            ...
            if include_dm:
                ...
                if users:
                    ...
                    user_dm_html_file_path = self.bookmark_generator.generate_user_dm_bookmarks(users, user_dm_output_file)
                    if user_dm_html_file_path:
                        ...
                        if user_guide_path:
                            ...
                            try:
                                ...
                                logger.info(f"ユーザーDMガイドページを生成しました: {user_guide_path}")
                            except Exception as e:
                                logger.error(f"ブラウザでファイルを開く際にエラーが発生しました: {e}")
                        else:
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--version", 
        action="version", 
        version=f"Slack to Bookmark v{__version__}"
    )
